...
for method in methods:
    for attribute, class_list in class_list_by_attribute.items():
        ...
        script = base_script.format(
            img_folder=f"results/celeba_multi/4/{method}/{attribute}",
            class_list=class_list
        )
        
        jobfile = Path(f"jobfiles/celeba_multi/4/{method}/evaluation/{attribute}.sh")
