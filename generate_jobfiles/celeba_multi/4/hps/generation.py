...
prompt_template = "a headshot of {smile} {age} {gender} {eyeglass}"
ages = ['old', 'young']
genders = ['man', 'woman']
smiling = ["a smiling", "a non-smiling"]
eyeglasses = ["with eyeglasses", "without eyeglasses"]
...
for idx_age, age in enumerate(ages):
    for idx_gender, gender in enumerate(genders):
        for idx_smile, smile in enumerate(smiling):
            for idx_eyeglass, eyeglass in enumerate(eyeglasses):
                combination = ""
                ...
                outdir = f"results/celeba_multi/4/hps/{attr_list}/{combination}"
                prompt = prompt_template.format(smile=smile, age=age, gender=gender, eyeglass=eyeglass)
                
                script = base_script.format(prompt=prompt, outdir=outdir, seed=seed)
                jobfile = Path(f"jobfiles/celeba_multi/4/hps/generation/{attr_list}/{combination}.sh")
