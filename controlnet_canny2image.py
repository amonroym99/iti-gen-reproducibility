...
def process(input_image, prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, low_threshold, high_threshold, sample_path):
    with torch.no_grad():
        ...
        img = Image.fromarray((255 - detected_map).astype(np.uint8))
...
def main():
    ...
    # Arguments ControlNet Canny Edge
    parser.add_argument(
        "--input_image",
        type=str,
        nargs="?",
        default="./test_imgs/human.png",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Canny Edge low threshold (from 1 to 255)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Canny Edge high threshold (from 1 to 255)",
    )
    ...
    for folder, index in folder_with_indexes.items():
        ...
        process(input_image, prompt, opt.n_prompt, opt.num_samples,
                opt.image_resolution, opt.ddim_steps, opt.guess_mode,
                opt.strength, opt.scale, opt.seed, opt.eta, opt.low_threshold, opt.high_threshold, sample_path)
